...
def read_config_json():
    ...
    config_path = os.getenv("MULTI_DOCKER_CONFIG")

    if not config_path:
        # If environment variable is not set, use a default config file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(script_dir, "client-config-for-multi-docker.json")
        print(f"⚠️  MULTI_DOCKER_CONFIG not set. Falling back to: {config_path}")
