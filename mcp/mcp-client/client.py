...
"""
Đây là một chương trình Python đóng vai trò như một MCP Client, giúp kết nối với một MCP server (terminal_server.py ), sau đó sử dụng Google Gemini API để:

- Tiếp nhận câu hỏi từ người dùng,

- Gọi các công cụ đã đăng ký (tool) ở MCP server nếu cần,

- Trả lại câu trả lời cuối cùng từ Gemini, có hoặc không kèm tool calling.

Cấu trúc chương trình gồm:

| Thành phần                      | Mục đích                                                           |
| ------------------------------- | ------------------------------------------------------------------ |
|   MCPClient                     | Lớp chính để kết nối MCP server, gửi yêu cầu, và quản lý chat loop |
|   convert_mcp_tools_to_gemini() | Chuyển tool từ MCP format sang format mà Google Gemini hiểu        |
|   clean_schema()                | Dọn dẹp schema JSON (loại bỏ `title`) để hợp lệ với Gemini         |
|   main()                        | Hàm chính để khởi chạy ứng dụng                                    |
|   Google Gemini                 | Là LLM engine chính, có khả năng gọi function (tool) nếu thấy cần  |
|   MCP server                    | Chứa các tool mà Gemini có thể gọi (qua `call_tool`)               |
...
class MCPClient:
    def __init__(self):
        ...
        # Khởi tạo client của Gemini (Google Generative AI SDK). 
        # API key lấy từ .env file (dùng dotenv).
        self.genai_client = genai.Client(api_key=gemini_api_key) 

    async def connect_to_server(self, server_script_path: str):
        """Connect to the MCP server and list available tools."""
        """
            - Xác định script server là .py hay .js → dùng python hay node để chạy.

            - Kết nối với MCP server bằng standard input/output (stdio).

            - Khởi tạo session ClientSession để gửi lệnh tới MCP server.

            - Lấy danh sách tool mà server đã đăng ký.

            - Chuyển đổi danh sách tool thành định dạng FunctionDeclaration cho Gemini.
        """
            ...
            if candidate.content.parts:  # Ensure response has content
                for part in candidate.content.parts:
                    if isinstance(part, types.Part):  # Check if part is a valid Gemini response unit
                        # - Kiểm tra nếu Gemini muốn gọi hàm:
                        #                 → Nếu có:
                        #                     Lấy tool_name và tool_args
                        #                     Gọi self.session.call_tool(tool_name, tool_args) đến MCP server
                        #                     Trả kết quả về cho Gemini như một message từ role "tool"
                        #                     Yêu cầu Gemini tiếp tục trả lời sau khi biết kết quả tool

                        #                 → Nếu không gọi tool:
                        #                     → Trả lại luôn nội dung văn bản từ Gemini.
    ...
    async def chat_loop(self):
        ...
        """ Chạy giao diện terminal: hỏi – trả lời theo vòng lặp, cho đến khi người dùng gõ quit."""
    ...
    async def cleanup(self):
        ...
        """ Đóng tất cả tài nguyên bất đồng bộ đúng cách (exit_stack.aclose())."""
        await self.exit_stack.aclose()

def clean_schema(schema):
    """
        Hàm phụ để xóa "title" trong JSON schema, vì Google Gemini yêu cầu format schema đơn giản và chuẩn JSON Schema draft.
    """
...
def convert_mcp_tools_to_gemini(mcp_tools):
    ...
    """
    """
        Chuyển mỗi MCP tool (gồm name, description, inputSchema) thành:
        Tool(function_declarations=[FunctionDeclaration(...)])
        Điều này rất quan trọng để Gemini biết và hiểu được các function có thể gọi.
    """
    gemini_tools = []
...
async def main():
    ...
    """
        Lấy server_script_path từ sys.argv[1], khởi chạy MCPClient, kết nối server, rồi gọi chat_loop().
                        [User Input] 
                            ↓
                        Gemini xử lý prompt
                            ↓
                        → Nếu cần function: gọi tool từ MCP server
